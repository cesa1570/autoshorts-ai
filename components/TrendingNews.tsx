...
import { Newspaper, Flame, ArrowRight, RefreshCw, Loader2, Globe, MapPin, Clock, ExternalLink, Video, Share2, X, Plus, Trophy, Skull, Tv, Cpu, MoreHorizontal } from 'lucide-react';
...
// Category options
const CATEGORIES = [
  { id: 'all', name: 'ทั้งหมด', icon: Newspaper },
  { id: 'sports', name: 'กีฬา', icon: Trophy },
  { id: 'crime', name: 'อาชญากรรม', icon: Skull },
  { id: 'entertainment', name: 'บันเทิง', icon: Tv },
  { id: 'tech', name: 'เทคโนโลยี', icon: Cpu },
  { id: 'other', name: 'อื่นๆ', icon: MoreHorizontal },
];
...
const TrendingNews: React.FC<TrendingNewsProps> = ({ onSelectTopic, apiKey }) => {
  ...
  const loadNews = async (isLoadMore: boolean = false) => {
    ...
    try {
      const items = await fetchTrendingNews(apiKey, activeRegion, activeCategory);
  ...
  useEffect(() => {
    loadNews(false);
  }, [apiKey, activeRegion, activeCategory]);

  return (
    <div className="space-y-6 relative">
      {/* Header Controls */}
      <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 backdrop-blur-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map(cat => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition ${activeCategory === cat.id

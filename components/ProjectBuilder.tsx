...
const ProjectBuilder: React.FC<{ initialTopic?: string, initialLanguage?: 'Thai' | 'English' }> = ({ initialTopic, initialLanguage = 'Thai' }) => {
   ...
   return (
      <div className="flex flex-col lg:flex-row gap-12 max-w-[1300px] mx-auto animate-in slide-in-from-bottom-8 duration-700">
         ...
         {/* 1. Vertical Preview Center (Clean Layout) */}
         <div className="lg:w-[400px] shrink-0 flex flex-col gap-6">
            <div className="relative bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/10 aspect-[9/16] group">
               ...
               {/* Overlay Controls */}
               {state.script && (
                  <div className="absolute bottom-6 right-6 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-all z-20">
                     <button onClick={() => playerRef.current?.renderVideo().then(({ blob }) => setCurrentVideoBlob(blob))} className="w-12 h-12 bg-emerald-600 text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition"><Download size={20} /></button>
                     <button onClick={() => setShowYoutubeModal(true)} disabled={!currentVideoBlob} className="w-12 h-12 bg-red-600 text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition disabled:opacity-50"><Youtube size={20} /></button>
                  </div>
               )}
            </div>

            <div className="flex items-center justify-between text-[#C5A059] px-2">
               <span className="text-[10px] font-black uppercase tracking-[0.2em]">Viral Preview 9:16</span>
               <div className="flex gap-2">
                  <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500">Live Render</span>
               </div>
            </div>
         </div>
         ...
         <div className="flex-1 flex flex-col gap-8">
            <div className="bg-[#0a0a0a] border border-white/5 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden ring-1 ring-white/5">
               <div className="absolute top-0 right-0 p-8 opacity-[0.03] pointer-events-none"><Zap size={140} fill="currentColor" /></div>

               <div className="flex items-center gap-4 mb-10">
                  <div className="w-14 h-14 rounded-2xl bg-[#C5A059]/10 flex items-center justify-center text-[#C5A059] shadow-lg border border-[#C5A059]/20"><Zap size={28} fill="currentColor" /></div>
                  <div>
                     <h2 className="text-3xl font-black text-white uppercase tracking-tight leading-none">Viral Lab</h2>
                     <span className="text-[10px] text-[#C5A059] font-black uppercase tracking-[0.3em] mt-2 block">High-Frequency Engagement Engine</span>
                  </div>
               </div>
